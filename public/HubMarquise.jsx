...
function HubAdmin({ onClose, hubSystems, setHubSystems }) {
  ...
  const [aba, _setAba] = useState(() => {
    try {
      ...
      return ['contas', 'setores', 'links', 'historico', 'feriados', 'ausencias', 'escala-spa'].includes(v) ? v : 'contas';
    } catch { return 'contas'; }
  });
  ...
  const ABAS = [
    ...
    { id: 'escala-spa', label: 'Escala SPA' },
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 95, background: HUB_PALETTE.noite, overflowY: 'auto', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ maxWidth: 900, margin: '0 auto', padding: isPhone ? '16px 12px 80px' : isMobile ? '24px 18px 80px' : '48px 48px 80px', width: '100%' }}>
        ...
        {/* ── Aba Escala SPA ── */}
        {aba === 'escala-spa' && <EscalaSpaPanel isMobile={isMobile} />}
...
// ─── Escala SPA (Gantt mensal de férias dos profissionais) ──────────────────
const MESES_PT_SPA = ['Janeiro','Fevereiro','Março','Abril','Maio','Junho','Julho','Agosto','Setembro','Outubro','Novembro','Dezembro'];
const DIAS_SEM_SPA = ['D','S','T','Q','Q','S','S'];

function EscalaSpaPanel({ isMobile }) {
  const hoje = new Date();
  const [mes, setMes] = useState(hoje.getMonth());
  const [ano, setAno] = useState(hoje.getFullYear());
  const [profissionais, setProfissionais] = useState(null);
  const [todasFerias, setTodasFerias] = useState([]);
  ...
  async function carregarProfissionais() {
    try {
      const r = await hubFetch('/api/admin/massagistas');
      const d = await r.json();
      if (!d.ok) throw new Error(d.erro || 'Erro ao carregar profissionais');
      setProfissionais((d.items || []).filter(m => !!m.ativo));
    } catch (e) {
      setLoadErr(e.message || 'Erro de conexão');
      setProfissionais([]);
    }
  }
  ...
  async function carregarFerias() {
    try {
      const r = await hubFetch('/api/admin/ferias');
      const d = await r.json();
      if (d.ok) setTodasFerias(d.ferias || []);
    } catch {}
  }

  useEffect(() => { carregarProfissionais(); carregarFerias(); }, []);
  ...
  function estaDeFerias(profId, dia) {
    const ds = `${ano}-${String(mes + 1).padStart(2,'0')}-${String(dia).padStart(2,'0')}`;
    return todasFerias.some(f => String(f.massagista_id) === String(profId) && f.data_inicio <= ds && f.data_fim >= ds);
  }

  const COL_NOME = isMobile ? 110 : 180;
  const COL_DIA  = isMobile ? 22  : 28;

  return (<>
    <div style={{ marginBottom: 28 }}>
      <div style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: 10, letterSpacing: '0.35em', textTransform: 'uppercase', color: HUB_PALETTE.champanhe, marginBottom: 14, display: 'flex', alignItems: 'center', gap: 12 }}>
        <span style={{ width: 18, height: 1, background: HUB_PALETTE.champanhe }} />Escala SPA
      </div>
      <h2 style={{ fontFamily: "'Helvetica Neue', Helvetica, Arial, sans-serif", fontWeight: 300, fontStyle: 'italic', fontSize: 40, letterSpacing: '-0.02em', color: HUB_PALETTE.marfim, margin: '0 0 10px' }}>Férias dos profissionais.</h2>
      <p style={{ fontFamily: 'Inter, sans-serif', fontSize: 14, color: HUB_PALETTE.areiaDim, lineHeight: 1.5, margin: 0 }}>
        Clique no nome do profissional para gerenciar os períodos de férias. Dias em dourado indicam ausência programada.
      </p>
    </div>

    {/* Month navigator */}
    <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 24 }}>
      <button onClick={prevMes} style={{ background: 'transparent', border: `1px solid ${HUB_PALETTE.areiaDim}44`, color: HUB_PALETTE.areia, width: 36, height: 36, cursor: 'pointer', fontSize: 18, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>‹</button>
      <span style={{ fontFamily: "'Helvetica Neue', Helvetica, Arial, sans-serif", fontSize: isMobile ? 18 : 22, color: HUB_PALETTE.marfim, fontWeight: 300, minWidth: isMobile ? 150 : 200, textAlign: 'center' }}>
        {MESES_PT_SPA[mes]} {ano}
      </span>
      <button onClick={nextMes} style={{ background: 'transparent', border: `1px solid ${HUB_PALETTE.areiaDim}44`, color: HUB_PALETTE.areia, width: 36, height: 36, cursor: 'pointer', fontSize: 18, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>›</button>
    </div>

    {loadErr && <div style={{ color: '#E07A5F', fontFamily: 'Inter, sans-serif', fontSize: 14, marginBottom: 20 }}>{loadErr}</div>}
    {profissionais === null && !loadErr && <div style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: 11, letterSpacing: '0.2em', color: HUB_PALETTE.areiaDim, textTransform: 'uppercase' }}>Carregando...</div>}
    {Array.isArray(profissionais) && profissionais.length === 0 && !loadErr && (
      <div style={{ fontFamily: "'Helvetica Neue', Helvetica, Arial, sans-serif", fontStyle: 'italic', fontSize: 16, color: HUB_PALETTE.areiaDim }}>Nenhum profissional ativo encontrado.</div>
    )}

    {Array.isArray(profissionais) && profissionais.length > 0 && (
      <div style={{ overflowX: 'auto', paddingBottom: 8 }}>
        {/* Header: day numbers */}
        <div style={{ display: 'flex', minWidth: COL_NOME + diasNoMes * COL_DIA }}>
          <div style={{ width: COL_NOME, flexShrink: 0 }} />
          {Array.from({ length: diasNoMes }, (_, i) => i + 1).map(d => {
            const isWk = [0,6].includes(new Date(ano, mes, d).getDay());
            return (
              <div key={d} style={{ width: COL_DIA, flexShrink: 0, textAlign: 'center', fontFamily: 'JetBrains Mono, monospace', fontSize: 9, color: isWk ? HUB_PALETTE.champanhe : HUB_PALETTE.areiaDim, paddingBottom: 1 }}>
                {d}
              </div>
            );
          })}
        ...
        <div style={{ display: 'flex', minWidth: COL_NOME + diasNoMes * COL_DIA, marginBottom: 6 }}>
          <div style={{ width: COL_NOME, flexShrink: 0, fontFamily: 'JetBrains Mono, monospace', fontSize: 8, letterSpacing: '0.2em', textTransform: 'uppercase', color: HUB_PALETTE.areiaDim + '66', display: 'flex', alignItems: 'center', paddingLeft: 0 }}>Profissional</div>
          {Array.from({ length: diasNoMes }, (_, i) => i + 1).map(d => {
            const dow = new Date(ano, mes, d).getDay();
            const isWk = dow === 0 || dow === 6;
            return (
              <div key={d} style={{ width: COL_DIA, flexShrink: 0, textAlign: 'center', fontFamily: 'JetBrains Mono, monospace', fontSize: 8, color: isWk ? HUB_PALETTE.champanhe + 'aa' : HUB_PALETTE.areiaDim + '66', paddingBottom: 4 }}>
                {DIAS_SEM_SPA[dow]}
              </div>
            );
          })}
        ...
        {profissionais.map(prof => (
          <div key={prof.id} style={{ display: 'flex', minWidth: COL_NOME + diasNoMes * COL_DIA, borderTop: `1px solid ${HUB_PALETTE.areiaDim}22` }}>
            <div
              style={{ width: COL_NOME, flexShrink: 0, padding: '10px 8px 10px 0', cursor: 'pointer', display: 'flex', flexDirection: 'column', justifyContent: 'center' }}
              onClick={() => setDrawerProf(prof)}
              title="Gerenciar férias"
            >
              <div style={{ fontFamily: 'Inter, sans-serif', fontSize: 13, color: HUB_PALETTE.marfim, fontWeight: 500, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                {prof.nome.split(' ')[0]}
              </div>
              {prof.funcao && (
                <div style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: 8, letterSpacing: '0.12em', color: HUB_PALETTE.areiaDim, textTransform: 'uppercase', marginTop: 2, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                  {prof.funcao}
                </div>
              )}
            </div>
            {Array.from({ length: diasNoMes }, (_, i) => i + 1).map(d => {
              const deFerias = estaDeFerias(prof.id, d);
              const isWk = [0,6].includes(new Date(ano, mes, d).getDay());
              const ds = `${ano}-${String(mes+1).padStart(2,'0')}-${String(d).padStart(2,'0')}`;
              const feriasRec = deFerias ? todasFerias.find(f => String(f.massagista_id) === String(prof.id) && f.data_inicio <= ds && f.data_fim >= ds) : null;
              return (
                <div
                  key={d}
                  onClick={() => setDrawerProf(prof)}
                  title={deFerias && feriasRec ? `Férias: ${feriasRec.data_inicio} → ${feriasRec.data_fim}` : undefined}
                  style={{
                    width: COL_DIA, flexShrink: 0, height: 44, cursor: 'pointer',
                    background: deFerias ? HUB_PALETTE.champanhe + 'cc' : isWk ? HUB_PALETTE.areiaDim + '0c' : 'transparent',
                    borderLeft: `1px solid ${HUB_PALETTE.areiaDim}15`,
                  }}
                />
              );
            })}
        ...
        <div style={{ height: 1, background: HUB_PALETTE.areiaDim + '22', minWidth: COL_NOME + diasNoMes * COL_DIA }} />
      </div>
    )}
    ...
    {drawerProf && (
      <FeriasDrawer
        prof={drawerProf}
        isMobile={isMobile}
        onClose={() => { setDrawerProf(null); carregarFerias(); }}
      />
    )}
  </>);
}

function FeriasDrawer({ prof, isMobile, onClose }) {
  const [lista, setLista] = useState(null);
  const [editing, setEditing] = useState(null);
  const [creating, setCreating] = useState(false);
  const [confirmar, setConfirmar] = useState(null);
  ...
  async function carregar() {
    try {
      const r = await hubFetch(`/api/admin/ferias?massagista_id=${prof.id}`);
      const d = await r.json();
      if (d.ok) setLista(d.ferias || []);
      else { setLista([]); notify(d.erro || 'Erro ao carregar', true); }
    } catch { setLista([]); notify('Erro de conexão', true); }
  }

  useEffect(() => { carregar(); }, [prof.id]);

  function fechar() { setEditing(null); setCreating(false); setErro(''); }

  async function salvar(campos) {
    setSaving(true); setErro('');
    const url = editing ? `/api/admin/ferias/${editing.id}` : '/api/admin/ferias';
    const method = editing ? 'PUT' : 'POST';
    try {
      const r = await hubFetch(url, {
        method,
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ ...campos, massagista_id: prof.id, massagista_nome: prof.nome }),
      });
      const d = await r.json().catch(() => ({}));
      if (!r.ok || !d.ok) { setErro(d.erro || `Erro ${r.status}`); setSaving(false); return; }
      notify(editing ? 'Férias atualizadas' : 'Férias programadas');
      fechar();
      await carregar();
    } catch { setErro('Erro de conexão'); }
    setSaving(false);
  ...
  async function confirmarExclusao() {
    ...
    setConfirmar(null);
    try {
      const r = await hubFetch(`/api/admin/ferias/${id}`, { method: 'DELETE' });
      const d = await r.json().catch(() => ({}));
      if (!r.ok || !d.ok) { notify(d.erro || 'Erro ao excluir', true); return; }
  ...
  const cs = {
    input: { width: '100%', boxSizing: 'border-box', background: HUB_PALETTE.areiaDim + '0a', border: `1px solid ${HUB_PALETTE.areiaDim}33`, color: HUB_PALETTE.marfim, fontFamily: 'Inter, sans-serif', fontSize: 14, padding: '10px 14px', outline: 'none' },
    label: { fontFamily: 'JetBrains Mono, monospace', fontSize: 10, letterSpacing: '0.25em', textTransform: 'uppercase', color: HUB_PALETTE.areiaDim, marginBottom: 6, display: 'block' },
    btnPrim: { background: HUB_PALETTE.champanhe, color: HUB_PALETTE.noite, border: 'none', padding: '10px 20px', fontFamily: 'JetBrains Mono, monospace', fontSize: 11, fontWeight: 600, letterSpacing: '0.2em', textTransform: 'uppercase', cursor: 'pointer' },
    btnGhost: { background: 'transparent', color: HUB_PALETTE.marfim, border: `1px solid ${HUB_PALETTE.areiaDim}77`, padding: '10px 20px', fontFamily: 'JetBrains Mono, monospace', fontSize: 11, fontWeight: 500, letterSpacing: '0.2em', textTransform: 'uppercase', cursor: 'pointer' },
    btnDanger: { background: 'transparent', color: '#E07A5F', border: '1px solid #E07A5F88', padding: '7px 12px', fontFamily: 'JetBrains Mono, monospace', fontSize: 9, fontWeight: 600, letterSpacing: '0.2em', textTransform: 'uppercase', cursor: 'pointer' },
    btnSm: { background: 'transparent', color: HUB_PALETTE.areia, border: `1px solid ${HUB_PALETTE.areiaDim}55`, padding: '7px 12px', fontFamily: 'JetBrains Mono, monospace', fontSize: 9, letterSpacing: '0.15em', textTransform: 'uppercase', cursor: 'pointer' },
  };

  const drawerW = isMobile ? '100vw' : '440px';
  ...
  return ReactDOM.createPortal(
    <div style={{ position: 'fixed', inset: 0, zIndex: 200, display: 'flex', alignItems: 'stretch' }}>
      ...
      <div style={{ width: drawerW, background: HUB_PALETTE.noite, borderLeft: `1px solid ${HUB_PALETTE.areiaDim}33`, display: 'flex', flexDirection: 'column', overflowY: 'auto' }}>
        {/* Header */}
        <div style={{ padding: '24px 24px 16px', borderBottom: `1px solid ${HUB_PALETTE.areiaDim}22`, flexShrink: 0 }}>
          <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 12 }}>
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: 9, letterSpacing: '0.3em', textTransform: 'uppercase', color: HUB_PALETTE.champanhe, marginBottom: 4 }}>Férias Programadas</div>
              <div style={{ fontFamily: "'Helvetica Neue', Helvetica, Arial, sans-serif", fontWeight: 300, fontStyle: 'italic', fontSize: 22, color: HUB_PALETTE.marfim, lineHeight: 1.2, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{prof.nome}</div>
              {prof.funcao && <div style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: 9, letterSpacing: '0.2em', textTransform: 'uppercase', color: HUB_PALETTE.areiaDim, marginTop: 4 }}>{prof.funcao}</div>}
            </div>
            <button onClick={onClose} style={{ background: 'transparent', border: `1px solid ${HUB_PALETTE.areiaDim}44`, color: HUB_PALETTE.areiaDim, width: 32, height: 32, cursor: 'pointer', fontSize: 18, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>×</button>
          </div>
        </div>
        ...
        <div style={{ flex: 1, padding: '20px 24px', overflowY: 'auto' }}>
          {lista === null && (
            <div style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: 11, letterSpacing: '0.2em', color: HUB_PALETTE.areiaDim, textTransform: 'uppercase' }}>Carregando...</div>
          )}
          {Array.isArray(lista) && lista.length === 0 && !creating && !editing && (
            <div style={{ fontFamily: "'Helvetica Neue', Helvetica, Arial, sans-serif", fontStyle: 'italic', fontSize: 15, color: HUB_PALETTE.areiaDim, marginBottom: 20 }}>
              Nenhum período programado.
            </div>
          )}
          {Array.isArray(lista) && lista.map(f => (
            <div key={f.id} style={{ padding: '14px 0', borderBottom: `1px solid ${HUB_PALETTE.areiaDim}22`, display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
              <div style={{ flex: 1, minWidth: 140 }}>
                <div style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: 12, color: HUB_PALETTE.champanhe, letterSpacing: '0.05em' }}>
                  {fmtData(f.data_inicio)} → {fmtData(f.data_fim)}
                </div>
                {f.observacao && (
                  <div style={{ fontFamily: 'Inter, sans-serif', fontSize: 12, color: HUB_PALETTE.areiaDim, marginTop: 4, lineHeight: 1.4 }}>{f.observacao}</div>
                )}
              </div>
              <button onClick={() => { fechar(); setEditing(f); }} style={cs.btnSm}>Editar</button>
              <button onClick={() => setConfirmar({ id: f.id, periodo: `${fmtData(f.data_inicio)} – ${fmtData(f.data_fim)}` })} style={cs.btnDanger}>Excluir</button>
            </div>
          ))}

          {(creating || editing) && (
            <FeriasForm isMobile={isMobile} cs={cs} erro={erro} saving={saving}
              initial={editing || null} isEdit={!!editing}
              onCancel={fechar} onSave={salvar} />
          )}

          {!creating && !editing && (
            <button onClick={() => { fechar(); setCreating(true); }} style={{ ...cs.btnPrim, marginTop: 20, width: '100%', textAlign: 'center' }}>
              + Programar férias
            </button>
          )}
        </div>
        ...
        {confirmar && (
          <div style={{ position: 'fixed', inset: 0, zIndex: 210, background: 'rgba(0,0,0,0.65)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
            <div style={{ background: HUB_PALETTE.noite, border: `1px solid ${HUB_PALETTE.areiaDim}33`, maxWidth: 380, width: '100%', padding: '28px 32px' }}>
              <div style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: 9, letterSpacing: '0.25em', textTransform: 'uppercase', color: '#9C5843', marginBottom: 6 }}>Confirmar exclusão</div>
              <h3 style={{ fontFamily: "'Helvetica Neue', Helvetica, Arial, sans-serif", fontStyle: 'italic', fontWeight: 300, fontSize: 20, color: HUB_PALETTE.marfim, margin: '0 0 12px' }}>
                Excluir período {confirmar.periodo}?
              </h3>
              <p style={{ fontFamily: 'Inter, sans-serif', fontSize: 13, color: HUB_PALETTE.areia, margin: '0 0 20px' }}>Esta ação não pode ser desfeita.</p>
              <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end' }}>
                <button onClick={() => setConfirmar(null)} style={cs.btnGhost}>Cancelar</button>
                <button onClick={confirmarExclusao} style={{ ...cs.btnPrim, background: '#E07A5F', color: '#fff' }}>Excluir</button>
              </div>
        ...
        {toast && (
          <div style={{ position: 'fixed', bottom: 32, left: '50%', transform: 'translateX(-50%)', zIndex: 220, background: toast.err ? '#E07A5F' : HUB_PALETTE.champanhe, color: toast.err ? '#fff' : HUB_PALETTE.noite, fontFamily: 'JetBrains Mono, monospace', fontSize: 11, letterSpacing: '0.18em', textTransform: 'uppercase', padding: '12px 22px' }}>
            {toast.msg}
          </div>
        )}
      </div>
...
function FeriasForm({ isMobile, cs, erro, saving, initial, isEdit, onCancel, onSave }) {
  const [dataInicio, setDataInicio] = useState(initial ? initial.data_inicio : '');
  const [dataFim, setDataFim]       = useState(initial ? initial.data_fim    : '');
  const [obs, setObs]               = useState(initial ? initial.observacao  : '');
  const inicioRef = useRef(null);

  useEffect(() => {
    const t = setTimeout(() => { if (inicioRef.current) inicioRef.current.focus(); }, 60);
    return () => clearTimeout(t);
  }, []);

  return (
    <div style={{ marginTop: 24, padding: '20px 0', borderTop: `1px solid ${HUB_PALETTE.areiaDim}33` }}>
      <div style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: 9, letterSpacing: '0.25em', textTransform: 'uppercase', color: HUB_PALETTE.champanhe, marginBottom: 16 }}>
        {isEdit ? 'Editar período' : 'Novo período de férias'}
      </div>
      <form autoComplete="off"
        onSubmit={e => { e.preventDefault(); onSave({ data_inicio: dataInicio, data_fim: dataFim, observacao: obs }); }}
        onKeyDown={e => { if (e.key === 'Escape' && !saving) { e.preventDefault(); onCancel(); } }}>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 12 }}>
          <div>
            <label style={cs.label}>Início *</label>
            <input ref={inicioRef} type="date" style={cs.input} value={dataInicio}
              onChange={e => setDataInicio(e.target.value)} required />
          </div>
          <div>
            <label style={cs.label}>Fim *</label>
            <input type="date" style={cs.input} value={dataFim}
              min={dataInicio || undefined}
              onChange={e => setDataFim(e.target.value)} required />
          </div>
        </div>
        <div style={{ marginBottom: 14 }}>
          <label style={cs.label}>Observação</label>
          <input style={cs.input} value={obs} maxLength={200}
            onChange={e => setObs(e.target.value)}
            placeholder="Ex: Férias de julho" />
        </div>
        {erro && (
          <div style={{ marginBottom: 12, padding: '10px 12px', border: '1px solid #E07A5F66', color: '#E07A5F', fontSize: 13, fontFamily: 'Inter, sans-serif' }}>
            {erro}
          </div>
        )}
        <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end' }}>
          <button type="button" onClick={onCancel} disabled={saving} style={cs.btnGhost}>Cancelar</button>
          <button type="submit" disabled={saving} style={cs.btnPrim}>{saving ? '...' : (isEdit ? 'Salvar' : 'Programar')}</button>
        </div>
      </form>
    </div>
  );
